...
# # ⚠️目的：处理tos_ssp126的数据，把txt文件转成excel文件夹中
# import os
# import re
# import pandas as pd
#
# # 定义文件路径
# input_folder = r'C:\A. climate change and mariculture\ArcGIS\data\conclim\tos'
# output_folder = r'C:\A. climate change and mariculture\Data_Process\climate_tos\126'
#
# # 创建输出文件夹（如果不存在）
# if not os.path.exists(output_folder):
#     os.makedirs(output_folder)
#
# # 正则表达式模式匹配 'tos_all_' 开头的文件名
# pattern = re.compile(r'tos_ssp126_all_.*\.txt')
#
# # 遍历输入文件夹中的所有TXT文件
...
# import os
#
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\126'
#
# # 获取文件夹中所有Excel文件的文件名
...
# # ⚠️目的：将tos_ssp126_eez_{year}.{month}.xlsx 按年份合并数。挑选出一年中最高温和最低温月份盐度值。
# import os
# import pandas as pd
#
# # 文件夹路径
...
#
# # 获取所有符合命名规则的文件
# file_pattern = "tos_eez_all_{year}.{month}.xlsx"
# files = [f for f in os.listdir(folder_path) if f.startswith("tos_eez_all") and f.endswith(".xlsx")]
#
# # 按年份合并文件
# yearly_files = {}
# for file in files:
...
#
#     # 按'i'和'j'分组，计算每个月份'tos'的最大值和最小值
...
#
#     # 保存结果到新的Excel文件
#     output_path = os.path.join(folder_path, f'tos_ssp126_eez_m_{year}.xlsx')
#     result_grouped.to_excel(output_path, index=False, sheet_name='tos_min_max')
#
# # -------------------------------------------section2.1补充tos空白值3s----------------------------------------------------
# # ⚠️目的：补充无tos数据的点位。
...
# # 定义文件路径
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\126'
# points_files = glob.glob(os.path.join(folder_path, 'tos_ssp126_eez_m_*.xlsx'))
#
#
# # 定义函数来填补空白值
# def fill_missing_values(df):
#     # 获取需要填补的列
#     cols_to_fill = ['tos_min', 'tos_max']
#
...
#
#     # 定义输出文件路径
#     output_file_path = os.path.join(folder_path, f'tos_ssp126_eez_m_filled_{year}.xlsx')
#
#     # 保存填补后的结果到Excel文件
...
# # 定义文件路径
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\126'
# species_file_path = r'C:\A. climate change and mariculture\Data_Process\species\species_tos_q.xlsx'
#
# # 读取物种数据
# species_df = pd.read_excel(species_file_path)
#
# # 获取所有年份的点位数据文件路径
# points_files = glob.glob(os.path.join(folder_path, 'tos_ssp126_eez_m_filled_*.xlsx'))
#
# # 对于每个年份的点位数据文件
# for file_path in points_files:
#     # 获取年份
...
#         i, j, point_tos_min, point_tos_max = point['i'], point['j'], point['tos_min'], point['tos_max']
#
#         # 对于每个物种
#         for _, species in species_df.iterrows():
#             species_no, species_name, species_tos_min, species_tos_max = species['No.'], species['species'], species[
#                 'tos_q10'], species['tos_q90']
#
#             # 判断物种是否能在该点位生存
#             suitable = species_tos_min <= point_tos_min and species_tos_max >= point_tos_max
#
#             result = {
#                 'i': i,
#                 'j': j,
...
#
#     # 定义输出文件路径
#     output_file_path = os.path.join(folder_path, f'tos_ssp126_eez_filled_species_q_{year}.xlsx')
#
#     # 保存结果到Excel文件
#     with pd.ExcelWriter(output_file_path) as writer:
...
# # 不需每块代码单独运算
# # ---------------------------------------section1.1气候数据格式数据处理50min---------------------------------------------------
# # ⚠️目的：处理tos_ssp585的数据，把txt文件转成excel文件夹中
# import os
# import re
# import pandas as pd
#
# # 定义文件路径
# input_folder = r'C:\A. climate change and mariculture\ArcGIS\data\conclim\tos'
# output_folder = r'C:\A. climate change and mariculture\Data_Process\climate_tos\585'
#
# # 创建输出文件夹（如果不存在）
# if not os.path.exists(output_folder):
#     os.makedirs(output_folder)
#
# # 正则表达式模式匹配 'tos_all_' 开头的文件名
# pattern = re.compile(r'tos_ssp585_all_.*\.txt')
#
# # 遍历输入文件夹中的所有TXT文件
# for filename in os.listdir(input_folder):
#     if pattern.match(filename):
...
# import os
#
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\585'
#
# # 获取文件夹中所有Excel文件的文件名
# excel_files = [f for f in os.listdir(folder_path) if f.endswith('.xlsx') or f.endswith('.xls')]
#
...
#
# # ----------------------------------------section1.4原始数据格式数据处理3s--------------------------------------------------
# # ⚠️目的：将tos_ssp585_eez_{year}.{month}.xlsx 按年份合并数。挑选出一年中最高温和最低温月份盐度值。
# import os
# import pandas as pd
#
# # 文件夹路径
...
#
#     # 保存结果到新的Excel文件
#     output_path = os.path.join(folder_path, f'tos_ssp585_eez_m_{year}.xlsx')
#     result_grouped.to_excel(output_path, index=False, sheet_name='tos_min_max')
#
# # -------------------------------------------section2.1补充tos空白值3s----------------------------------------------------
# # ⚠️目的：补充无tos数据的点位。
...
# # 定义文件路径
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\585'
# points_files = glob.glob(os.path.join(folder_path, 'tos_ssp585_eez_m_*.xlsx'))
#
#
# # 定义函数来填补空白值
# def fill_missing_values(df):
...
#
#     # 定义输出文件路径
#     output_file_path = os.path.join(folder_path, f'tos_ssp585_eez_m_filled_{year}.xlsx')
#
#     # 保存填补后的结果到Excel文件
#     with pd.ExcelWriter(output_file_path) as writer:
#         filled_df.to_excel(writer, index=False)
...
# # 每块代码单独运算
# # ---------------------------------------section1.1气候数据格式数据处理50min---------------------------------------------------
# # ⚠️目的：处理tos_historical的数据，把txt文件转成excel文件夹中
# import os
# import re
# import pandas as pd
#
# # 定义文件路径
# input_folder = r'C:\A. climate change and mariculture\ArcGIS\data\conclim\tos'
# output_folder = r'C:\A. climate change and mariculture\Data_Process\climate_tos\historical'
#
# # 创建输出文件夹（如果不存在）
# if not os.path.exists(output_folder):
#     os.makedirs(output_folder)
#
# # 正则表达式模式匹配 'tos_all_' 开头的文件名
# pattern = re.compile(r'tos_historical_all_.*\.txt')
#
# # 遍历输入文件夹中的所有TXT文件
# for filename in os.listdir(input_folder):
#     if pattern.match(filename):
...
# import os
#
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\historical'
#
# # 获取文件夹中所有Excel文件的文件名
# excel_files = [f for f in os.listdir(folder_path) if f.endswith('.xlsx') or f.endswith('.xls')]
#
...
#
# # ----------------------------------------section1.4原始数据格式数据处理3s--------------------------------------------------
# # ⚠️目的：将tos_historical_eez_{year}.{month}.xlsx 按年份合并数。挑选出一年中最高温和最低温月份盐度值。
# import os
# import pandas as pd
#
# # 文件夹路径
...
#
#     # 保存结果到新的Excel文件
#     output_path = os.path.join(folder_path, f'tos_historical_eez_m_{year}.xlsx')
#     result_grouped.to_excel(output_path, index=False, sheet_name='tos_min_max')
#
# # -------------------------------------------section2.1补充tos空白值3s----------------------------------------------------
# # ⚠️目的：补充无tos数据的点位。
...
# # 定义文件路径
# folder_path = r'C:\A. climate change and mariculture\Data_Process\climate_tos\historical'
# points_files = glob.glob(os.path.join(folder_path, 'tos_historical_eez_m_*.xlsx'))
#
#
# # 定义函数来填补空白值
# def fill_missing_values(df):
...
#
#     # 定义输出文件路径
#     output_file_path = os.path.join(folder_path, f'tos_historical_eez_m_filled_{year}.xlsx')
#
#     # 保存填补后的结果到Excel文件
#     with pd.ExcelWriter(output_file_path) as writer:
#         filled_df.to_excel(writer, index=False)
...
# # -------------------------------------------⚠️new5个年次判断：section4.1物种tos判断最大最小平均值10s------------------------------------------------------
# # 接2.1后面，新的条件来判断---------------------------
# import pandas as pd
# import os
#
...
#     result_df = combined_df.groupby(['i', 'j']).agg({'tos_min': 'min', 'tos_max': 'max'}).reset_index()
#
#     # 保存结果到新的Excel文件中
...
#
# # 文件夹路径
# input_folder = r'C:\A. climate change and mariculture\Data_Process\old\climate_tos'
# output_folder = r'C:\A. climate change and mariculture\Data_Process\new'
#
# # 调用函数处理不同的文件
# process_files(os.path.join(input_folder, 'historical'), output_folder, 'tos_historical_eez_m_filled_',
#               'tos_historical_5year.xlsx', '5year')
# process_files(os.path.join(input_folder, '126'), output_folder, 'tos_ssp126_eez_m_filled_202',
#               'tos_ssp126_20-24_5year.xlsx', '20-24_5year')
# process_files(os.path.join(input_folder, '126'), output_folder, 'tos_ssp126_eez_m_filled_203',
#               'tos_ssp126_30-34_5year.xlsx', '30-34_5year')
# process_files(os.path.join(input_folder, '126'), output_folder, 'tos_ssp126_eez_m_filled_204',
#               'tos_ssp126_40-44_5year.xlsx', '40-44_5year')
# process_files(os.path.join(input_folder, '126'), output_folder, 'tos_ssp126_eez_m_filled_205',
#               'tos_ssp126_50-54_5year.xlsx', '50-54_5year')
# process_files(os.path.join(input_folder, '585'), output_folder, 'tos_ssp585_eez_m_filled_202',
#               'tos_ssp585_20-24_5year.xlsx', '20-24_5year')
# process_files(os.path.join(input_folder, '585'), output_folder, 'tos_ssp585_eez_m_filled_203',
#               'tos_ssp585_30-34_5year.xlsx', '30-34_5year')
# process_files(os.path.join(input_folder, '585'), output_folder, 'tos_ssp585_eez_m_filled_204',
#               'tos_ssp585_40-44_5year.xlsx', '40-44_5year')
# process_files(os.path.join(input_folder, '585'), output_folder, 'tos_ssp585_eez_m_filled_205',
#               'tos_ssp585_50-54_5year.xlsx', '50-54_5year')
